...
%% (iii) Minimization of an artificial potential
...
xlabel("X (m)")
ylabel("Y (m)")
title('Open-loop Control using V')
subtitle('Square')
legend
grid on
grid minor
...
V_dot=-[2*(theta(1)-30) ; 2*0.25*(theta(2)-90) ; 2*0.66*(theta(3)-60);0 ;0];

% Theta dot total
theta_d_dot= (180/pi)*pinv(J)*(Xd_dot) + (eye(5)- (pinv(J)*J))*V_dot;
