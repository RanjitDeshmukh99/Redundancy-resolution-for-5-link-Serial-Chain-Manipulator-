...
%% ii) Ellipse Closed-loop task-space control
...
[time theta_sol]=ode45(@taskspacecontrol,[0,3*20], theta0);
...
xlabel("X (m)")
ylabel("Y (m)")
title('Closed-loop task-Space control')
subtitle('Ellipse')
legend
grid on
grid minor
...
ylabel("Y (m)")
title(' Animation')
subtitle('Closed-loop task-Space control : Ellipse')
...
function [theta_d_dot]= taskspacecontrol(t,theta)
...
% All angles 0 to 360 deg
theta(1:5)=wrapTo360(theta(1:5));

% Forward Kinematics end effector co-ordinates
X(1) = L1.*cosd(theta(1)) + L2.*cosd(theta(2)) + L3.*cosd(theta(3)) + L4.*cosd(theta(4)) + L5.*cosd(theta(5));
X(2) = L1.*sind(theta(1)) + L2.*sind(theta(2)) + L3.*sind(theta(3)) + L4.*sind(theta(4)) + L5.*sind(theta(5));
...
% Theta dot total
theta_d_dot= pinv(J)*(Xd_dot - K*[Xd(1)-X(1) ; Xd(2)-X(2)]);
